// Banner de X (1500x500): Colombia → Celo, versión panorámica
...
const W = 1500, H = 500;
// Globo a la izquierda (recortado abajo, estilo banner)
const cx = 330, cy = 330, r = 250;
const projection = geoOrthographic().scale(r).translate([cx, cy]).rotate([65, 0]);
...
const BX = 900, BY = 150, BS = 140;  // badge Celo
const P2 = [BX - BS / 2 + 8, BY + BS / 2 - 8];
const C = [(P0[0] + P2[0]) / 2 + 30, Math.min(P0[1], P2[1]) - 130];
...
const svg = `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  <defs>
    <radialGradient id="bgGlow" cx="40%" cy="50%" r="75%">
    ...
    <linearGradient id="prism" x1="0%" y1="100%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#22d3ee"/><stop offset="35%" stop-color="#22c55e"/>
      <stop offset="60%" stop-color="#fbbf24"/><stop offset="100%" stop-color="#e879f9"/>
    </linearGradient>
    <linearGradient id="routeCelo" x1="0%" y1="100%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#22d3ee"/><stop offset="55%" stop-color="#22c55e"/><stop offset="100%" stop-color="#FCFF52"/>
    </linearGradient>
    <radialGradient id="ocean" cx="42%" cy="36%" r="72%">
      <stop offset="0%" stop-color="#15204a"/><stop offset="100%" stop-color="#070b18"/>
    </radialGradient>
    ...
    <clipPath id="badgeClip"><rect x="${BX - BS / 2}" y="${BY - BS / 2}" width="${BS}" height="${BS}" rx="26"/></clipPath>
    ...
    <filter id="route" x="-60%" y="-60%" width="220%" height="220%"><feGaussianBlur stdDeviation="8"/></filter>
    <pattern id="grid" width="56" height="56" patternUnits="userSpaceOnUse">
      <path d="M 56 0 L 0 0 0 56" fill="none" stroke="rgba(255,255,255,0.055)" stroke-width="1"/>
    </pattern>
  </defs>

  <rect width="${W}" height="${H}" fill="url(#bgGlow)"/>
  <rect width="${W}" height="${H}" fill="url(#grid)"/>
  <ellipse cx="760" cy="300" rx="560" ry="230" fill="url(#prism)" opacity="0.14" filter="url(#soft)"/>

  <g fill="#ffffff">
    <circle cx="1120" cy="70" r="2" opacity="0.6"/><circle cx="1380" cy="120" r="1.6" opacity="0.5"/>
    <circle cx="700" cy="60" r="1.6" opacity="0.5"/><circle cx="1460" cy="300" r="2" opacity="0.5"/>
    <circle cx="620" cy="440" r="1.6" opacity="0.4"/><circle cx="1180" cy="440" r="1.6" opacity="0.4"/>
  </g>

  <circle cx="${cx}" cy="${cy}" r="${r + 12}" fill="none" stroke="url(#prism)" stroke-width="1.5" opacity="0.35"/>
  <circle cx="${cx}" cy="${cy}" r="${r}" fill="url(#ocean)" stroke="rgba(255,255,255,0.16)" stroke-width="2.5"/>
  <g clip-path="url(#globeClip)">
    <path d="${gratD}" fill="none" stroke="rgba(255,255,255,0.09)" stroke-width="1.5"/>
    <path d="${landD}" fill="#3a5170" fill-opacity="0.72" stroke="rgba(255,255,255,0.16)" stroke-width="1.1"/>
    <path d="${colombiaD}" fill="url(#colFlag)" stroke="#ffffff" stroke-width="2"/>
  </g>

  <path d="${routeD}" fill="none" stroke="url(#routeCelo)" stroke-width="22" stroke-linecap="round" opacity="0.5" filter="url(#route)"/>
  <path d="${routeD}" fill="none" stroke="url(#routeCelo)" stroke-width="12" stroke-linecap="round" stroke-dasharray="2 22"/>

  <circle cx="${P0[0]}" cy="${P0[1]}" r="24" fill="#FCD116" opacity="0.35"/>
  <circle cx="${P0[0]}" cy="${P0[1]}" r="13" fill="#FCD116"/>
  <circle cx="${P0[0]}" cy="${P0[1]}" r="6" fill="#ffffff"/>

  <rect x="${BX - BS / 2 - 12}" y="${BY - BS / 2 - 12}" width="${BS + 24}" height="${BS + 24}" rx="32" fill="#FCFF52" opacity="0.16" filter="url(#soft)"/>
  <image href="${celoHref}" x="${BX - BS / 2}" y="${BY - BS / 2}" width="${BS}" height="${BS}" preserveAspectRatio="xMidYMid slice" clip-path="url(#badgeClip)"/>
  <rect x="${BX - BS / 2}" y="${BY - BS / 2}" width="${BS}" height="${BS}" rx="26" fill="none" stroke="#FCFF52" stroke-width="3"/>

  ${robot(1360, 300, 0.95)}

  <text x="${P0[0]}" y="${P0[1] + 44}" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="bold" fill="#ffffff" text-anchor="middle">Colombia</text>
  <text x="${BX}" y="${BY + BS / 2 + 40}" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="bold" fill="#FCFF52" text-anchor="middle">Celo</text>
...
writeFileSync("public/banner-x.svg", svg, "utf-8");
console.log("banner-x.svg generado (1500x500)");
